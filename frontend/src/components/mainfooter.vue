    ...
    <footer class="bg-gray-800 text-white py-12 relative">
        <div class="container mx-auto px-4">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div v-motion :initial="{ opacity:0, y:100,}":visibleOnce="{opacity:1, y:0,}">
              <h3 class="text-2xl font-bold mb-4">EpicTrails Travels</h3>
              <p class="text-gray-400">Crafting unforgettable journeys since 2010</p>
            </div>
            <div v-motion :initial="{ opacity:0, y:100,}":visibleOnce="{opacity:1, y:0,}">
              <h4 class="text-lg font-semibold mb-4">Quick Links</h4>
              <ul class="space-y-2">
                <li v-for="item in navItems" :key="item"><a :href="`#${item.toLowerCase()}`" class="text-gray-400 hover:text-white transition duration-200">{{ item }}</a></li>
              </ul>
            </div>
            <div v-motion :initial="{ opacity:0, y:100,}":visibleOnce="{opacity:1, y:0,}">
